function plane = ReadPlane(planeName, year, directory)
%% All missions
% Text file import as table
opts = detectImportOptions(fullfile([directory '\Competitions\' year '\Planes\' planeName '.txt']));
opts.VariableOptions(1,2) = opts.VariableOptions(1,1);
txtParams = readtable(fullfile([directory '\Competitions\' year '\Planes\' planeName '.txt']), opts);
...
plane.wingThicknessLocation = airfoil.wingThicknessLocation;
plane.CLgroundRoll = params(6);

plane.hWinglet = params(7);
...
plane.lengthPayload3 = params(57);
