...
while nLaps < lapLimit && timeAtLapEnd < timeLimit && segment.performance.s(end) > (1-usableBatteryFraction) && ~segment.performance.stall(end)
    ...
    % 180
    segment = PropulsionModel(plane,environment,missionNo,turnThrottle,turnCL,'turn180',0,0,segment.coursePoints,segment.performance);
    ...
    % 360
    segment = PropulsionModel(plane,environment,missionNo,turnThrottle,turnCL,'turn360',0,0,segment.coursePoints,segment.performance);
...
function turnThrot = findTurnThrot(segment)
    ...
    turnThrot = median(segment.performance.throttle(prevStartIndx:prevEndIndx));
